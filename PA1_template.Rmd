...
hist(stepsPerDay,col="red",breaks=5,main="Histogram of Steps per Day",xlab = "Steps per Day")
...
In this dataset, the mean number of steps taken per day is `r format(meanStepsPerDay,nsmall = 2)` and the median is `r medianStepsPerDay`.
...
For this dataset, the subject is most active between 8:35 and 8:40, with an average 206.17 steps in that interval.
...
we copy our data into a new dataset.  Then we add a column to the table for the
...
median has become the mean.  The histrogram distribution is almost the same, except
...
on weekdays and weekends.  However, they move more consistently on weekends througout the
